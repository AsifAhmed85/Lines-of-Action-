...
bool isInCluster(int cluster[8][8], int n, int x, int y, int cluster_no)
{
    if(x+1<n && cluster[x+1][y]==cluster_no)
        return true;
    if(x-1>=0 && cluster[x-1][y]==cluster_no)
        return true;
    if(y+1<n && cluster[x][y+1]==cluster_no)
        return true;
    if(y-1>=0 && cluster[x][y-1]==cluster_no)
        return true;
    if(x+1<n && y+1<n && cluster[x+1][y+1]==cluster_no)
        return true;
    if(x+1<n && y-1>=0 && cluster[x+1][y-1]==cluster_no)
        return true;
    if(x-1>=0 && y+1<n && cluster[x-1][y+1]==cluster_no)
        return true;
    if(x-1>=0 && y-1>=0 && cluster[x-1][y-1]==cluster_no)
        return true;

    return false;
}

int howManyClusterAround(int cluster[8][8], int n, int x, int y)
{
    set<int> clst;
    if(x+1<n && cluster[x+1][y] != -1)
        clst.insert(cluster[x+1][y]);
    if(x-1>=0 && cluster[x-1][y] != -1)
        clst.insert(cluster[x-1][y]);
    if(y+1<n && cluster[x][y+1] != -1)
        clst.insert(cluster[x][y+1]);
    if(y-1>=0 && cluster[x][y-1] != -1)
        clst.insert(cluster[x][y-1]);
    if(x+1<n && y+1<n && cluster[x+1][y+1] != -1)
        clst.insert(cluster[x+1][y+1]);
    if(x+1<n && y-1>=0 && cluster[x+1][y-1] != -1)
        clst.insert(cluster[x+1][y-1]);
    if(x-1>=0 && y+1<n && cluster[x-1][y+1] != -1)
        clst.insert(cluster[x-1][y+1]);
    if(x-1>=0 && y-1>=0 && cluster[x-1][y-1] != -1)
        clst.insert(cluster[x-1][y-1]);
    return clst.size();
}
...
int findDistFromCluster(char grid[8][8], int n, int cluster[8][8], int x, int y, int cluster_no)
{
    ...
    while(!que.empty())
    {
        ...
        if(isInCluster(cluster, n, point.first, point.second, cluster_no) || howManyClusterAround(cluster, n, point.first, point.second)>1)
        {
            dist = distance_array[point.first][point.second];
            //cout<<x<<" "<<y<<" "<<point.first<<" "<<point.second<<" "<<isInCluster(cluster, n, point.first, point.second, cluster_no)<<endl;
            break;
        }
        /*if(distance_array[point.first][point.second]>n/2)
            break;*/

        if(grid[point.first][point.second] == 'e') i = 1;
        else i = 0;
        up_down_color = getNoOfColor(grid, n, 0, point.second, 1, 0) + i;
        right_left_color = getNoOfColor(grid, n, point.first, 0, 0, 1) + i;
        diagonal1_color = getNoOfColor(grid, n, point.first-min(point.first,point.second), y-min(point.first,point.second), 1, 1) + i;
        diagonal2_color = getNoOfColor(grid, n, (point.first+point.second>=(n-1))? (n-1):(point.first+point.second), (point.first+point.second>=(n-1))?(point.first+point.second-(n-1)):0, -1, 1) + i;

        if((point.first+1)<n && isValidMove(grid, n, point.first, y, 1, 0, up_down_color, color) && visited[point.first+up_down_color][point.second]==0)
        {
            que.push(make_pair(point.first+up_down_color, point.second));
            visited[point.first+up_down_color][point.second] = 1;
            distance_array[point.first+up_down_color][point.second] = distance_array[point.first][point.second] + 1;
        }
        if(isValidMove(grid, n, point.first, point.second, -1, 0, up_down_color, color) && visited[point.first-up_down_color][point.second]==0)
        {
            que.push(make_pair(point.first-up_down_color, point.second));
            visited[point.first-up_down_color][point.second] = 1;
            distance_array[point.first-up_down_color][point.second] = distance_array[point.first][point.second] + 1;
        }
        if(isValidMove(grid, n, point.first, point.second, 0, 1, right_left_color, color) && visited[point.first][point.second+right_left_color]==0)
        {
            que.push(make_pair(point.first, point.second+right_left_color));
            visited[point.first][point.second+right_left_color] = 1;
            distance_array[point.first][point.second+right_left_color] = distance_array[point.first][point.second] + 1;
        }
        if(isValidMove(grid, n, point.first, point.second, 0, -1, right_left_color, color) && visited[point.first][point.second-right_left_color]==0)
        {
            que.push(make_pair(point.first, point.second-right_left_color));
        ...
        if(isValidMove(grid, n, point.first, point.second, 1, 1, diagonal1_color, color) && visited[point.first+diagonal1_color][point.second+diagonal1_color]==0)
        {
            que.push(make_pair(point.first+diagonal1_color, point.second+diagonal1_color));
            visited[point.first+diagonal1_color][point.second+diagonal1_color] = 1;
            distance_array[point.first+diagonal1_color][point.second+diagonal1_color] = distance_array[point.first][point.second] + 1;
        }
        if(isValidMove(grid, n, point.first, point.second, 1, -1, diagonal2_color, color) && visited[point.first+diagonal2_color][point.second-diagonal2_color]==0)
        {
            que.push(make_pair(point.first+diagonal2_color, point.second-diagonal2_color));
            visited[point.first+diagonal2_color][point.second-diagonal2_color] = 1;
            distance_array[point.first+diagonal2_color][point.second-diagonal2_color] = distance_array[point.first][point.second] + 1;
        }
        if(isValidMove(grid, n, point.first, point.second, -1, 1, diagonal2_color, color) && visited[point.first-diagonal2_color][point.second+diagonal2_color]==0)
        {
            que.push(make_pair(point.first-diagonal2_color, point.second+diagonal2_color));
...
void alphaBeta(char grid[8][8], int n, int depth)
{
    ...
    for(int i=0; i<moves.size(); i++)
    {
        ...
        changed_grid[branch.first.first][branch.first.second] = 'e';
        changed_grid[branch.second.first][branch.second.second] = my_color;
        //cout<<branch.first.first<<" "<<branch.first.second<<" "<<branch.second.first<<" "<<branch.second.second<<endl;
        temp = minVal(changed_grid, n, depth-1, alpha, beta);
        if(temp>alpha)
        {
            alpha = temp;
